...
/* function slider()
{
    ...
    // console.log("filted by " + slider.value);
    // console.log("first element in original products ", JSON.stringify(originalProducts[0]));    
    // let filteredP = originalProducts.filter(product => (product.final_price <= slider.value));
    let filteredP = originalProducts.filter(product => (parseFloat(product.final_price) <= parseFloat(slider.value)));

    // if (Array.isArray(filteredP) && filteredP.length > 0) {
    //     // Log before passing to displayProducts to make sure it's correct
    //     // console.log("Passing filtered products to displayProducts: ", JSON.stringify(filteredP));
    //         console.log("here"); 
    //     // Call displayProducts with the filtered products
  
        displayProducts(filteredP);
    // } else {
    //     console.error("Error: filteredP is not an array or is empty.");
    // }
